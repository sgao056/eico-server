...
Eico.create = (newEico, result) => {
  sql.query(`INSERT INTO POSTS (delta, text, created, edited, viewed, pined) VALUES(${newEico.delta}, ${newEico.text}, ${newEico.created}, ${newEico.edited}, ${newEico.viewed}, ${
    newEico.pined ? 1:0 
  })`, (err, res) => {
    if (err) {
...
Eico.updateById = (id, eico, result) => {
  sql.query(
    `UPDATE POSTS SET delta = ${eico.delta}, text = ${eico.text}, created = ${eico.created}, edited = ${eico.edited}, pined = ${eico.pined ? 1:0 }, viewed = ${eico.viewed} WHERE id = ${id}`,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
